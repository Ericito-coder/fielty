        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {tarjetas.map(t => (
            <button key={t.id} onClick={() => window.location.href = `/tarjeta/${t.id}`}
              style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px', background: theme.bgMuted2, border: 'none', borderRadius: 14, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: theme.black }}>{t.negocioNombre}</div>
                <div style={{ fontSize: 12, color: theme.gray, marginTop: 2 }}>Ver mi tarjeta →</div>
              </div>
            </button>
          ))}
        </div>
        <div style={{ textAlign: 'center', marginTop: 20 }}>
          <button style={{ fontSize: 13, color: theme.gray, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
