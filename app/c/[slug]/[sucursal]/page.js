...
export default function CajaSlugSucursal({ params }) {
  ...
  async function acreditarPuntos() {
    ...
    await supabase.from('transacciones').insert([{
      ...
      tipo: 'suma',
      puntos: pts,
      descripcion: `Compra $${valor.toLocaleString()} — ${sucursal.nombre}`
    }])
  ...
  async function validarCanje() {
    ...
    if (new Date() > new Date(canje.expira_at)) {
      ...
      const { data: clienteActual } = await supabase.from('clientes').select('puntos').eq('id', canje.clientes.id).single()
      await supabase.from('clientes').update({ puntos: (clienteActual?.puntos || 0) + canje.puntos_descontados }).eq('id', canje.clientes.id)
  ...
  if (!negocio || !sucursal) return <div style={{minHeight:'100vh', background:'#0e0e0e'}} />

  if (pantalla === 'pin') return (
    <div style={s.wrap}>
      <div style={s.pinWrap}>
        <div style={{...s.ppLogo, background: negocio.color}}>{negocio.nombre.slice(0,2).toUpperCase()}</div>
        <div style={s.pinTitle}>{negocio.nombre}</div>
        <div style={s.pinSub}>📍 {sucursal.nombre}</div>
        <div style={s.pinDots}>
          {[0,1,2,3].map(i => (
    ...
    <div style={s.wrap}>
      ...
      <div style={s.topbar}>
        <div style={s.topbarLeft}>
          <div style={{...s.ppLogoSm, background: negocio.color}}>{negocio.nombre.slice(0,2).toUpperCase()}</div>
          <div>
            <div style={s.topbarBiz}>Caja · {sucursal.nombre}</div>
      ...
      <div style={s.content}>
            ...
            {clientes.map(c => {
              ...
              return (
                <div key={c.id} style={s.clientRow} onClick={() => seleccionarCliente(c)}>
                  <div style={{...s.clientAvatar, background: negocio.color}}>{c.nombre.slice(0,2).toUpperCase()}</div>
                  <div style={{flex:1}}>
        ...
        {busqueda.length < 2 && (
          <div style={{marginTop:24}}>
            ...
            <button style={s.quickBtn} onClick={() => setPantalla('validar')}>🎁 Validar canje de recompensa</button>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div style={s.wrap}>
        ...
        <div style={s.content}>
          ...
          <button style={{...s.btnGhost, marginTop:10}} onClick={() => setPantalla('validar')}>🎁 Validar canje de recompensa</button>
        </div>
      </div>
    )
  }
  ...
  if (pantalla === 'validar') return (
    <div style={s.wrap}>
      ...
      <div style={s.topbar}>
        ...
        <div style={s.topbarBiz}>Validar canje — {sucursal.nombre}</div>
      </div>
      <div style={s.content}>
...
const s = {
  ...
  pinSub: { fontSize:14, color:'#888', marginTop:-16 },
