...
export default function CajaSlugSucursal({ params }) {
  ...
  if (isMobile === null || !negocio || !sucursal) return <div style={{minHeight:'100vh', background:theme.black}} />
  ...
  if (pantalla === 'pin') return (
    <div style={{minHeight:'100vh', background:theme.black, color:'white', display:'flex', alignItems:'center', justifyContent:'center'}}>
      ...
      <main style={{display:'flex', flexDirection:'column', alignItems:'center', gap:24, padding:32}}>
        ...
        <div style={{fontSize:14, color:theme.gray, marginTop:-16}}>📍 {sucursal.nombre}</div>
        <input
          type="password"
  ...
  if (!isMobile) return (
    <div style={{minHeight:'100vh', background:theme.black, color:'white', display:'flex', flexDirection:'column'}}>
      ...
      {escanerJsx}
      {mensaje && <div style={s.toast(mensaje.tipo)}>{mensaje.texto}</div>}
      <header style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 28px', borderBottom:'1px solid #1e1e1e'}}>
        <div style={{display:'flex', alignItems:'center', gap:12}}>
          ...
          <span style={{background:'#1a1a1a', color:theme.grayMid, fontSize:11, padding:'3px 10px', borderRadius:100, border:'1px solid #2a2a2a'}}>📍 {sucursal.nombre}</span>
        </div>
