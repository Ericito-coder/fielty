...
export default function CajaSucursal({ params }) {
  ...
  useEffect(() => {
    params.then(p => {
      setNegocioId(p.negocio)
      setSucursalId(p.sucursal)
    })
  }, [params])
  ...
  function presionarPin(d) {
    ...
    if (nuevo.length === 4) {
      setTimeout(() => {
        ...
        if (!pinReal) { mostrarMensaje('❌ PIN no configurado. Configuralo en el dashboard', 'error'); setPin(''); return }
  ...
  async function confirmarCanje() {
    ...
    await supabase.from('canjes')
      .update({ estado: 'usado', usado_at: new Date().toISOString(), sucursal_id: sucursalId })
