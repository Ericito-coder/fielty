 ...
 * cliente: { id, nombre, puntos } — negocio: { id, nombre, color, logo_url }
 */
export async function generarLinkWallet(cliente, negocio) {
  const cfg = getConfig()
  if (!cfg) return null
  ...
  const objeto = {
    ...
    loyaltyPoints: { label: 'Puntos', balance: { int: cliente.puntos || 0 } },
    barcode: {
      type: 'QR_CODE',
...
/**
 * Actualiza el saldo del pase del cliente. Fire-and-forget: si el
 * cliente nunca agregó el pase (404) o Wallet no está configurado,
 * no pasa nada.
 */
export async function actualizarPuntosWallet(clienteId, puntos) {
  const cfg = getConfig()
  if (!cfg) return
  try {
    const token = await getAccessToken(cfg.sa)
    await fetch(`${API}/loyaltyObject/${objectId(cfg, clienteId)}`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ loyaltyPoints: { label: 'Puntos', balance: { int: puntos } } }),
    })
  } catch (error) {
