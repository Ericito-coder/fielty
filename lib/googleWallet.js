...
async function asegurarClase(cfg, token, negocio) {
  ...
  if (existe.ok) {
    ...
    // queda congelado con los datos del día que se creó.
    //
    // Se manda SOLO la presentación. Incluir `reviewStatus` acá reiniciaría
    // la revisión de Google, y borrar y recrear la clase perdería su
    // aprobación: por eso es un PATCH parcial y nunca un reemplazo.
    try {
