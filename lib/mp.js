...
export async function sincronizarSuscripcion(supabaseAdmin, negocioId) {
  const { data: negocio } = await supabaseAdmin
    .from('negocios')
    .select('id, plan, mp_plan_id, mp_plan_tipo')
    .eq('id', negocioId)
    .single()
  ...
  } else if (preapprovals.length && negocio.plan && negocio.plan !== 'gratis') {
  ...
  if (cambio) {
    await supabaseAdmin.from('negocios').update({ plan: planNuevo }).eq('id', negocioId)
  }
