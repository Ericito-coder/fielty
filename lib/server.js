...
export async function validarPinCaja(supabaseAdmin, { negocioId, slug, sucursalId, sucursalSlug, pin, ip }) {
  ...
  const { ok: rateLimitOk } = await rateLimit({ key: `pin:${ip || 'unknown'}`, maxAttempts: 10, windowMs: 15 * 60 * 1000 })
  if (!rateLimitOk) return { error: 'Demasiados intentos. Esperá 15 minutos.', status: 429 }
