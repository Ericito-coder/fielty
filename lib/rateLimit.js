...
/**
 * Consulta si una clave llegó al límite SIN sumar un intento. Para
 * flujos donde solo los fracasos deben contar (ej: PIN de caja): se
 * chequea acá antes de validar, y se suma con rateLimit() únicamente
 * cuando la validación falla.
 */
export async function rateLimitPeek({ key, maxAttempts = 5 }) {
  try {
    const supabaseAdmin = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL,
      process.env.SUPABASE_SERVICE_ROLE_KEY
    ...
    const { data, error } = await supabaseAdmin
      ...
      .maybeSingle()
    if (!error) {
      if (!data || new Date(data.reset_at) < new Date()) return { ok: true }
      return { ok: data.count < maxAttempts }
    }
  } catch { /* fallback abajo */ }

  const record = attempts.get(key)
  if (!record || Date.now() > record.resetAt) return { ok: true }
